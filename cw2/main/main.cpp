...
#	endif // ~ !NDEBUG
	...
	Shader lightingShader("assets/ml.vert", "assets/ml.frag");
	Shader lightCubeShader("assets/lc.vert", "assets/lc.frag");
	...
	unsigned int VBOLight, cubeVAOLight;
	glGenVertexArrays(1, &cubeVAOLight);
	...
	glBindVertexArray(cubeVAOLight);
	...
	*/
	...
	glUniform1i(glGetUniformLocation(skyboxprog.programId(), "skybox"), 0);
...
*/
		...
		unsigned int viewLoc1 = glGetUniformLocation(skyboxprog.programId(), "view");
		unsigned int projectionLoc1 = glGetUniformLocation(skyboxprog.programId(), "projection");

		glUniformMatrix4fv(viewLoc1, 1, GL_FALSE, &view1[0][0]);
		glUniformMatrix4fv(projectionLoc1, 1, GL_FALSE, &projection1[0][0]);
		// ... 设置观察和投影矩阵
		glBindVertexArray(skyboxVAO);
		...
		*/
		...
		glm::mat4 projectionLight = glm::perspective(glm::radians(camera.Zoom), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f);
		glm::mat4 viewLight = camera.GetViewMatrix();
		lightingShader.setMat4("projection", projectionLight);
		lightingShader.setMat4("view", viewLight);
		...
		lightCubeShader.setMat4("view", viewLight);
		...
		*/
		...
		//projection = glm::perspective(glm::radians(45.0f), (float)iwidth / (float)iheight, 0.1f, 100.0f);
		glm::mat4 projection = glm::perspective(glm::radians(camera.Zoom), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f);
		unsigned int viewLoc = glGetUniformLocation(prog.programId(), "view");
		unsigned int projectionLoc = glGetUniformLocation(prog.programId(), "projection");
...
void processInput(GLFWwindow* window)
{
	...
	if (glfwGetKey(window, GLFW_KEY_LEFT_CONTROL) == GLFW_PRESS)
		camera.ProcessSpeed(DOWN, deltaTime);
	if (glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS)
		camera.ProcessSpeed(UP, deltaTime);
...
* void processInput(GLFWwindow* window)
{
	...
	if (glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS)
		cameraSpeed *= 2;
	if (glfwGetKey(window, GLFW_KEY_E) == GLFW_PRESS)
		cameraPos += glm::normalize(glm::cross(cameraDown, cameraFront)) * cameraSpeed;
	if (glfwGetKey(window, GLFW_KEY_Q) == GLFW_PRESS)
		cameraPos -= glm::normalize(glm::cross(cameraDown, cameraFront)) * cameraSpeed;
	if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS)
